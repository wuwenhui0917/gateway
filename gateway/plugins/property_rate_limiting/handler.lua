...
local counter = require("gateway.plugins.property_rate_limiting.counter")
...
function PropertyRateLimitingHandler:access(conf)
    ...
    local enable = gateway_db.get(plugin_config.table_name..".enable")
    local meta = gateway_db.get_json(plugin_config.table_name..".meta")
    local selectors = gateway_db.get_json(plugin_config.table_name..".selectors")
