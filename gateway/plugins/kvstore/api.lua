...
API:post("/kvstore/enable", function(store)
    return function(req, res, next)
        ...
        if update_result then
            local success, err, forcible = gateway_db.set("kvstore.enable", enable)
            result = success
        else
            result = false
        end
...
API:post("/kvstore/sync", function(store)
    return function(req, res, next)
        ...
        local ss, err3, forcible = gateway_db.set("kvstore.enable", data.enable)
        ...
        ss, err3, forcible = gateway_db.set_json("kvstore.conf", data.conf)
        if not ss or err3 then
            return res:json({
                success = false,
...
API:get("/kvstore/configs", function(store)
    return function(req, res, next)
        res:json({
            success = true,
            data = {
                enable = gateway_db.get("kvstore.enable"),
                conf = gateway_db.get_json("kvstore.conf")
            }
        })
    end
end)
...
API:post("/kvstore/configs", function(store)
    return function(req, res, next)
        ...
        if update_result then
            local result, err, forcible = gateway_db.set("kvstore.conf", conf)
            success = result
            if success then
                data.conf = json.decode(conf)
                data.enable = gateway_db.get("kvstore.enable")
            end
        else
            success = false
        end
...
API:get("/kvstore/get", function(store)
    return function(req, res, next)
        ...
        local block = false
        local conf = gateway_db.get_json("kvstore.conf")
        if conf then
