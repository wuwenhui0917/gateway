...
local function _do_cmd(self, cmd, key, ...)
    ...
    for k=1, MAGIC_TRY do
        ...
        for i=1,#serv_list do
            ...
            if passwd and ok then
                ok,err = redis_client:auth(passwd)
                if err then
                    ngx.log(ngx.ERR,"getRedis  password error" )
                    return nil, "oops! redis password error"
    ...
    return nil, "oops! please contact asiainfo iisc wwh6 "
end
...
function _M.commit_pipeline(self)
    ...
    for k, v in pairs(map) do
        ...
        if passwd and ok then
            ok,err = ins:auth(passwd)
            if err then
                ngx.log(ngx.ERR,"getRedis  password error" )
                return nil, "getRedis  password error"
            end     
        end  
