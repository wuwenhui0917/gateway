...
--add wuwh 通过渠道ID查询规则
function _M.get_info_ByChanId(plugin, store, chanid,type)
    
    if not chanid or chanid == "" or type(chanid) ~= "string" then
        return nil
    end

    if not type or type == "" or type(type) ~= "string" then
        return nil
    end
   
   
    local rules, err = store:query({
        sql = "select * from " .. plugin .. " where  `type`=? and chanid=?",
        params = {type ,chanid}
    })
    if err then
...
function _M.create_pluginData(plugin, store, data,types,chanid,id)
    ngx.log(ngx.ERR,"data>>>>>>>>>>>>>>>>>>>>>>"..json.encode(data) .. chanid)
    return store:insert({
        --sql = "insert into " .. plugin .. "(`key`, `value`, `type`, `op_time`) values(?,?,?,?)",
